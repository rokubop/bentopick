...
1. In `bentopick.toml`, set `enabled = true` under `[browser]`. Restart BentoPick.
...
4. Put that origin in `browser.allow`. Restart BentoPick.

BentoPick only listens once `allow` is non-empty, so nothing connects before step 4.
...
Log lives at `%LOCALAPPDATA%\bentopick\bentopick.log`. A wrong origin says so:
...
The token lives in plaintext in `bentopick.toml`, so anything running as you can
